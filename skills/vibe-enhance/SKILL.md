...
📦 자동 추가한 라이브러리 (n개 — `[ENHANCE:DEP]`, 관례상 필수 / 현행 표준):
...
- **Every LARGE auto-apply must be reported in the dedicated 🆙 section** of Template 1, with fit citation, trade-off, and a clean revert command. **Every new-library auto-apply must be reported in the dedicated 📦 section** with library/version/license, standard ref URL, project-fit signal, swap-check confirmation, footprint trade-off, and an exact uninstall + revert command. Buried-in-the-small-list reporting for either is a failure of this skill.
