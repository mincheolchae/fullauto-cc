...
flowchart TD
    ...
    Loop --> Spawn["task당 claude -p spawn<br/>fresh 컨텍스트, /review-loop 가용"]
    Spawn --> Sub["하나의 task만 처리<br/>서브에이전트가 /review-loop을 호출해<br/>BLOCK 발견사항을 자체 수정 후 종료"]
    ...
    Done -.->|"--vibe-enhance 사용 시,<br/>한 기능 그룹 완료마다"| Vibe["/vibe-enhance 패스<br/>트렌드 비교 → 추가 적용<br/>→ /review-loop 검증"]
    Vibe -.-> Gates
```
...
찾아서 작은 ENHANCE만 자동 적용한 다음 `/review-loop`로 검증합니다.
...
- **`run` 모드 (대화형)**: 인터랙티브 터미널이면 진행 여부를 물어봅니다
  (누락된 env가 있으면 기본값이 N). 비-TTY 환경에서는 표시만 하고 통과 —
  강제로 막으려면 `--strict-prereqs`. 프롬프트 자체를 건너뛰려면 `--yes`(`-y`).
...
| `-y, --yes` | run | 수동 선결조건 confirm 프롬프트 건너뛰기 (auto는 항상 비대화) |
| `--strict-prereqs` | run | 비-TTY 환경에서 누락된 `[ENV]` 항목이 있으면 시작 거부 (auto는 placeholder 주입으로 우회) |
| `--vibe-enhance` | run / auto | 한 기능 그룹 완료마다 `/vibe-enhance` 패스 실행 — 트렌드 기반 개선 자동 추가 + `/review-loop` 검증. resume에서는 무시(이전 run의 설정이 우선) |
