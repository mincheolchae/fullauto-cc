...
export function buildSubagentPrompt(
  ...
  return [
    ...
    `2. **Tests are part of your task scope.** If your task adds testable runtime behavior (a new endpoint, API handler, service / business-logic function, database mutation, or non-trivial pure function), include at least one runtime test that exercises it as part of your work — same commit, same task. This is NOT scope creep; it is how the orchestrator's test gate (\`npm test\` / \`pytest\` / etc.) actually verifies your output rather than only catching type errors. The test must live in a place the project's existing test runner picks up automatically (e.g. \`tests/\`, \`__tests__/\`, \`*_test.go\`). Skip the test-add ONLY when one of these applies — and say briefly which one in your final message so the user knows it was deliberate:`,
    `   (a) Your task is itself a test task — title contains "test" / "spec" / "contract test" / "integration test", or body says "write a test for X". Write only the test, not the underlying implementation.`,
    `   (b) The task body or its sub-bullets explicitly delegate tests to a separate task — e.g. \`tests: T###\`, \`tested by: T###\`, "tests live in the test phase". Trust that arrangement and write only the implementation.`,
    `   (c) The task adds no testable runtime behavior — pure config / scaffolding (\`add dependency\`, \`create directory\`), UI styling / theming, documentation-only edits.`,
    `3. ${verifyLoopInstruction}`,
    `4. Do not run \`git commit\`, \`git push\`, or any destructive shell command unless the task explicitly requires it.`,
    `5. Verification gates (typecheck/test/lint) are run by the orchestrator AFTER you finish — you do not need to invoke them yourself, but your code (including the test you just wrote) must pass them.`,
    ``,
    `## Output protocol`,
...
export async function runSubagent(
  ...
  // Resolve --mcp-config path BEFORE entering the promise. See `resolveMcpArgs`
  // for the full vetting story (lexical + realpath containment so a symlink
  // escape can't leak absolute filesystem layout).
  const mcpArgs = await resolveMcpArgs(projectDir, config.mcpConfigPath);
